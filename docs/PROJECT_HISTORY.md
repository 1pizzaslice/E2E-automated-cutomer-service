...
- Milestone 6 channel intake is roughly half complete. The normalized inbound message contract is defined in `packages/shared-schemas` as `NormalizedInboundMessageSchema` (plus `NormalizedInboundChannelSchema`, `CustomerIdentityTypeSchema`, and customer-identity/body/attachment sub-schemas and inferred types; attachment size is nullable and a message must carry text, html, or an attachment). `packages/integrations/src/channels` adds pure provider adapters — `parseInboundEmailMessage` (email) and `parseInboundWhatsAppMessages` (WhatsApp Cloud, batched) — that map non-strict raw provider payloads into the strict normalized contract with attachment metadata and threading, plus timing-safe HMAC-SHA256 signature verification for WhatsApp (`X-Hub-Signature-256`) and Mailgun. Webhook/polling ingress endpoints, raw payload/attachment storage, inbound dedup persistence, conversation threading persistence, and workflow start/signal wiring remain later Milestone 6 slices.
...
Milestone 6 Channel Intake is ~half complete: the normalized inbound message schema, email/WhatsApp provider adapters, attachment metadata mapping, and provider signature verification are done in `packages/shared-schemas` and `packages/integrations/src/channels`. Continue with the ingress + persistence slices:

1. Add `POST /v1/webhooks/email/{provider}` and `POST /v1/webhooks/whatsapp/{provider}` webhook ingress in `packages/api` (plus an email polling placeholder). Each handler verifies the provider signature with the `packages/integrations` verifiers over the raw request body, stores the raw payload by reference, resolves tenant/channel, then calls the adapter.
2. Add raw payload storage by reference (object store abstraction / `raw_payload_ref`) and attachment storage handling.
3. Add inbound dedup/idempotency persistence keyed by `external_message_id` + tenant/channel so duplicate provider events do not create duplicate messages, and conversation threading persistence keyed by `external_thread_id`.
4. Wire normalized inbound intake to the ready `ticketLifecycleWorkflow` start/`message_received` signal boundary. Keep real provider send/download calls behind adapter boundaries until their contracts are ready.
