...
- The full v1 ticket lifecycle now runs end to end as one process on local infra with the deterministic AI model (Milestone 13): the production worker entrypoint composes real ticket/approval/outbound/audit persistence, deterministic triage + drafting, domain event emission, and telemetry; the Temporal workflow persists every workflow-owned ticket state transition and handles first-response SLA breach and approval expiry. Still pending on the launch path: the Python AI sidecar behind `runAiGraph` (Milestone 14), real model/embedding providers (Milestone 15), real auth (Milestone 16), scheduled jobs (Milestone 17), and next-response/resolution SLA timers.
...
Milestone 13 - Production Worker Entrypoint And Ticket Persistence (`feat-milestone13-worker-entrypoint-ticket-persistence`) is complete; all ten checklist items and three acceptance criteria are checked off in `TODO.md` (production ticket persistence activities, the explicit state-transition activity with ticket events + audits, the `messages` enum migration, approval expiry, the ai_run/tool_call completed domain events, the composed worker entrypoint with `pnpm worker:start`, and the committed live end-to-end test incl. the worker-restart no-duplicates acceptance — ADR-0021). Next: Milestone 14 - AI Runtime Service Bridge (checklist in `TODO.md`; platform decision in ADR-0020):

1. Add the FastAPI sidecar under `ai/service/` (`POST /internal/ai/run`, `GET /health`, internal bearer token via SecretResolver conventions; deps via a new uv extra).
2. Implement the production TypeScript `runAiGraph` activity calling the sidecar over HTTP (timeout + retryable-vs-permanent classification; sidecar unavailability → structured `failed` result routed to human) behind the unchanged `createPersistedRunAiGraph` wrapper — replacing the Milestone 13 deterministic TypeScript stand-in `createDeterministicRunAiGraph`.
3. Expose `POST /internal/tools/execute` in `packages/api` (Milestone 8 envelope, service-to-service auth) and implement the Python `ToolExecutor`/`RetrievalPort` adapters calling the live registry and `POST /v1/kb/search` in service mode.
4. Feed `PolicyContext` (`allow_auto_send`, allowed topics/tools) from `createDatabaseAutomationPolicyStore` into the `RuntimeRequest` built by the activity, propagate `correlation_id`/`trace_id`, add the sidecar to Compose, and prove service-path runs identical to in-process runs via the eval suite.
