...
- Milestone 7 KB and retrieval is ~half complete (the ingestion vertical). `packages/shared-schemas` adds the ingestion contracts: `KbDocumentCreateRequestSchema` (content is required and stored by reference), `KbDocumentUpdateRequestSchema` (metadata/status PATCH), `KbChunkResponseSchema` (embeddings intentionally omitted from the contract), and `KbIngestionResultSchema`. `packages/integrations/src/kb` adds the pure, deterministic pipelines: `chunkDocument` (overlapping paragraph-aware chunks) and an `Embedder` port with `createDeterministicEmbedder` (token-hash L2-normalized `vector(1536)` vectors). `packages/db` adds `0003_kb_vector_index.sql` (a pgvector HNSW `vector_cosine_ops` index over `kb_chunks.embedding`) plus tenant-scoped `createKbDocumentQuery`/`updateKbDocumentByIdQuery`/`deleteKbChunksForDocumentQuery`/`insertKbChunksQuery` helpers. `packages/api` adds the ingestion vertical: a `KbContentStore` port (filesystem default) that keeps raw bodies out of PostgreSQL, a tenant-scoped `KbIngestionStore` (writes under RLS; chunk replacement is atomic), a `KbIngestionService` that wires content storage + chunking + embedding + chunk writes, and the endpoints `POST /v1/kb/documents` (create as draft), `PATCH /v1/kb/documents/{id}` (metadata/status), and `POST /v1/kb/documents/{id}/ingest` (chunk, embed, activate) gated behind a new `kb_documents:write` permission. Verified end to end against a live pgvector database (draft → ingest → active, embedded chunks persisted, cosine `<=>` search ranks correctly). Tenant-scoped retrieval/search, citation metadata, stale/active exclusion, retrieval eval fixtures, prompt-injection content, and tenant-isolation retrieval tests remain the second (retrieval) half.
...
Milestone 6 Channel Intake is complete (webhook ingress, raw payload storage, dedup/idempotency, conversation threading, and workflow start/signal wiring all landed in `packages/api`; all checklist items and acceptance criteria are checked off in `TODO.md`). Begin Milestone 7 - KB And Retrieval:

1. Define the KB document and KB chunk schemas in `packages/shared-schemas` (the `kb_documents`/`kb_chunks` tables and the `pgvector` embedding column already exist in `packages/db`).
2. Add the document upload/ingestion API, chunking pipeline, and embedding pipeline, and create the pgvector index.
3. Add tenant-scoped retrieval with citation metadata that never crosses a tenant boundary and excludes stale/inactive documents.
4. Add retrieval evaluation fixtures, prompt-injection test content, and tenant-isolation retrieval tests.
