...
Milestone 18 - Staging Environment On Hardened Compose (`feat-milestone18-staging-hardened-compose`) is complete as code + config (ADR-0027): the `infra/production/` deployment — production Dockerfiles (pruned `pnpm deploy` images running `tsx` on source), the 15-service hardened Compose profile (only Caddy public, operator UIs `127.0.0.1`-bound, internal bridge, restart policies + resource limits, shared raw-payload volume, profile-gated migrate/seed), Caddy TLS fronting `/v1/*`+webhooks+`/health`, Prometheus/Grafana/Alertmanager provisioned from `infra/observability/` with a Slack receiver, git-ignored per-service env-file secrets with templates, nightly backups + a tested restore drill, and a `DEPLOY_ENABLED`-gated CI deploy workflow with health-gated `deploy.sh`/`rollback.sh` — all validated by a full local bring-up. Phase 1 (Milestones 13-17) plus the code half of Phase 2 (Milestone 18) are done.

Next: **Milestone 19 - Live Providers And Go-Live Rehearsal** (Phase 2), but it is gated on user-owned inputs — a provisioned staging VM + DNS to run the Milestone 18 stack, and a Mailgun account + verified pilot domain (WhatsApp waits on Meta Business verification). Steps once the VM + Mailgun exist: stand staging up per `infra/production/README.md`, close the VM-only SOPS §19 boxes, then configure Mailgun inbound → `POST /v1/webhooks/email/mailgun`, prove signature verification, round-trip a real approved reply, and run the full staging rehearsal + load/chaos smoke. **In parallel with no VM dependency: Milestone 20 - Console Enablement API** (CORS, approval-queue ergonomics + evidence composite, `ticket_events` read API, Redis rate limiting, `packages/api-client`, the route↔spec drift test) — unblocks Milestone 23 (the console) landing before the Milestone 22 go-live.

Milestone 18 follow-ups (not blockers): the S3/MinIO `RawPayloadStore`/`BlobSweeper` behind the existing ports stays deferred — API and worker share the `RAW_PAYLOAD_STORE_DIR` volume for now (ADR-0027(3)); the `pg-backup` scheduler is an in-container sleep-loop (swap for host cron/systemd if preferred); the offsite backup hook (`BACKUP_UPLOAD_CMD`) + MinIO `mc mirror` target are user-owned; the Caddyfile proxies only the API + webhooks until Milestone 23 adds a `console` route/service; the collector is pinned to `0.154.0` (the version the config's `translation_strategy` targets).
