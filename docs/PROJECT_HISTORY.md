...
Milestone 11 - Observability And QA (`feat-milestone11-observability-qa`) is complete; all its checklist items and acceptance criteria are checked off in `TODO.md`. A ticket is traceable end to end by one correlation id across request spans, activity spans, `ai_runs.trace_id`, and `audit_events.correlation_id`; QA reviewers get the full evidence package in one read; and the critical failure modes have metrics plus alert definitions. Begin Milestone 12 - Security And Pilot Readiness:

1. Verify/extend RBAC enforcement and add integration secret handling plus PII redaction in logs (the structured logger's key-based redaction is a start; log-content redaction policy is still open).
2. Add the prompt-injection test suite, attachment validation, and audit completeness tests.
3. Add data retention policy hooks, pilot tenant seed data, a pilot onboarding SOP, and the weekly review report query.
4. Add auto-send allowlist controls and the production deployment checklist. Acceptance: no known cross-tenant leakage, no high-risk action can bypass human approval, pilot metrics reportable, incident/escalation SOPs documented.
