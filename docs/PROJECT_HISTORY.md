...
Milestone 16 - Real Authentication And Policy Lifecycle (`feat-milestone16-real-auth-policy-lifecycle`) is complete (ADR-0024): production JWT auth via Clerk/JWKS at the `readAuthContext` choke point (jose verifier, `users.idp_subject` mapping with DB-sourced roles, server-side tenant membership, header mode only behind `SUPPORT_AUTH_MODE=insecure-headers`), the policy lifecycle endpoints (create/version/activate/archive with `policy.created|activated|archived` audits, activation immutability, one active policy per domain, admin-only `policies:write`, write+activation-time automation-content validation), `retention_policy` on the tenant contract, the RBAC matrix under real signed tokens with the full negative suites, the live-PostgreSQL auth+lifecycle integration suite, and the live Clerk smoke (real session token verified through the real JWKS).

Next: Milestone 17 - Scheduled Jobs And Retention Execution (checklist in `TODO.md`): Temporal Schedules for the per-tenant QA sampling and retention jobs (idempotent create-if-missing bootstrap from the worker entrypoint), the blob sweeper for cleared raw-payload refs, the attachment-purge and AI-run-anonymization retention classes (currently counted-and-reported placeholders), job observability + a job-failure alert rule, and live schedule coverage against local Temporal.
