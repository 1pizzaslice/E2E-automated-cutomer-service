...
Milestone 10 - Approval And Outbound Messaging (`feat-milestone10-approval-outbound`) is complete; all its checklist items and acceptance criteria are checked off in `TODO.md`. The approval decision endpoints resolve pending approvals, audit the edited-draft trail, and signal the Temporal workflow, which sends approved/edited responses exactly once through the real (idempotent, audited) outbound send activity. Begin Milestone 11 - Observability And QA:

1. Add OpenTelemetry tracing plus structured logs with trace ids across `packages/api` and `packages/workers` (the Compose stack already ships an OTel collector).
2. Add metrics for APIs, workflows, AI runs, tool calls, and approvals.
3. Add the QA review data model + sampling job and AI run trace links (this is also where `ai_runs` rows get persisted, unblocking the `approvals.ai_run_id`/`messages.ai_run_id` FK linking that Milestone 10 guards around).
4. Add basic dashboard definitions and alert definitions for critical failures; acceptance is an end-to-end traceable ticket and a QA review view over conversation, evidence, tool calls, AI output, human edits, and final response.

Milestone 10 follow-ups (not blockers): a production worker entrypoint composing `createTicketLifecyclePersistenceActivities` + `createDatabaseTicketLifecyclePersistenceStore` + `createHttpOutboundChannelSender` with the remaining placeholder activities into `createTicketLifecycleWorker`; a live-PostgreSQL workers integration test for `createDatabaseTicketLifecyclePersistenceStore`; migrating `messages.send_status`/`sent_by_type` from free text to PostgreSQL enums; an email subject strategy for outbound replies (currently null — providers thread via the reply headers); and approval expiry (`expired` status) handling.
