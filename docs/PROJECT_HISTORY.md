...
Milestone 17 - Scheduled Jobs And Retention Execution (`feat-milestone17-scheduled-jobs-retention`) is complete (ADR-0025): per-tenant daily Temporal Schedules for the QA sampling and retention jobs with idempotent create-if-missing bootstrap from the worker entrypoint, the fail-closed `BlobSweeper` (filesystem, sweep-before-clear), the executed attachment-purge and AI-run-anonymization retention classes (`anonymized_at`, migration `0007`), job metrics/spans/logs with the `SupportScheduledJobFailures`/`SupportRetentionSweepFailures` alert rules, and the committed live schedule suite (`pnpm test:jobs`) against real local Temporal. Phase 1 (run end-to-end, Milestones 13-17) is done.

Next: Milestone 18 - Staging Environment On Hardened Compose (checklist in `TODO.md`; Phase 2, ADR-0020): production Dockerfiles for API/worker/AI sidecar, the hardened Compose profile under `infra/production/` (restart policies, resource limits, internal-only networks), the TLS reverse proxy, Prometheus/Grafana/Alertmanager loading `infra/observability/` (including the new job alert rules), secrets as env files per the SecretResolver contract (incl. `SUPPORT_AUTH_ISSUER`/`SUPPORT_AUTH_AUDIENCE`, provider keys, and a consistent `RAW_PAYLOAD_STORE_DIR` or the S3/MinIO store + sweeper behind the same ports), nightly backups with a restore drill, and the CI deploy workflow with rollback. User-owned inputs: the VM, DNS records, and the alert notification channel.

Milestone 17 follow-ups (not blockers): Milestone 18 delivers the S3/MinIO object store + sweeper behind the same `RawPayloadStore`/`BlobSweeper` ports (until then API and worker must share `RAW_PAYLOAD_STORE_DIR`); schedule coverage lags tenant onboarding by one worker restart (consider a reconciler if tenant creation becomes API-frequent); `audit_event_days` stays deliberately unenforced (append-only compliance ledger) until a tenant contract requires audit purging; the WhatsApp media-download slice will start writing local attachment blobs that the attachment class then actually sweeps.
