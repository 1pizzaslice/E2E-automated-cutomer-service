...
- The live opt-in gate (costs tokens; requires the provider key): `SUPPORT_LLM_PROVIDER=anthropic SUPPORT_LLM_MODEL=claude-opus-4-8 PYTHONPATH=ai uv run --frozen --project ai --extra llm python -m evals.live_runner` — mandatory before serving traffic on a new provider/model/prompt configuration (SOPS §11.1). Real-provider deps install with `uv sync --project ai --extra llm`.
