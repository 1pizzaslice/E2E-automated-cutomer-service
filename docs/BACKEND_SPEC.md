...
- `packages/workers/src/domain-events.ts` provides worker-side emit helpers for `support.message.received.v1`, `support.ticket.created.v1`, and ticket state transition events. These helpers build schema-validated envelopes and publish through an injected `DomainEventPublisher`; they are intended for future Temporal workflow/activity code, not direct CRUD route side effects.
