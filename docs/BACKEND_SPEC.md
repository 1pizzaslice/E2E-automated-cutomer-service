...
The current tenant, customer, ticket, conversation, and message endpoints are
...
below; conversations and messages currently support read/list only. They validate
...
or ticket lifecycle transitions.
...
- Run live PostgreSQL integration tests for DB/RLS and API tenant/customer/conversation/message/ticket endpoints: `DATABASE_URL=postgres://support:support@localhost:5432/support pnpm test:integration`.
...
- Tenant-scoped entities have `tenant_id` columns and tenant indexes. Repository query helpers currently enforce tenant filters for customer and ticket reads/lists/updates/writes, conversation and message reads/lists, plus KB chunks, integrations, audit events, and tool definitions.
...
- Live repository execution tests seed two synthetic tenants and prove the tenant-scoped helpers execute against PostgreSQL without returning cross-tenant customers, conversations, messages, tickets, KB chunks, integrations, tool definitions, or audit events.
