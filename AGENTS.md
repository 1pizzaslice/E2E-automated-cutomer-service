...
- Run the live eval gate against a configured real model (opt-in, costs tokens): `SUPPORT_LLM_PROVIDER=anthropic SUPPORT_LLM_MODEL=claude-opus-4-8 PYTHONPATH=ai uv run --frozen --project ai --extra llm python -m evals.live_runner` (offline agnosticism proof: `SUPPORT_LLM_PROVIDER=scripted` with `--extra service`)
