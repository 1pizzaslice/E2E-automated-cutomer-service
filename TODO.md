...
2. **Milestone 23 follow-up (VM-side only):** bring the `console` service up on the VM alongside the Milestone 18 stack and extend the public-domain TLS to it (SOPS §19). The image builds and serves the SPA locally; only the VM deploy remains, folded into the Milestone 19/22 bring-up.
3. **Milestone 19** once the VM + Mailgun exist: stand up staging on the VM from the Milestone 18 artifacts (follow `infra/production/README.md` — fill `.env` + `env/*.env` with Clerk **production** issuer/audience, Anthropic/OpenAI keys, Slack webhook, strong datastore passwords, and the `CONSOLE_*` build args; bring up incl. the `console` service; migrate + seed; RLS smoke; SOPS §19), then configure the Mailgun inbound route → `POST /v1/webhooks/email/mailgun`, confirm signature verification (good + bad-signature 403), verify an approved draft round-trips to a real mailbox threaded via RFC 5322 headers, run the full staging rehearsal and the load/chaos smoke, and wire WhatsApp Cloud API when Meta verification clears.
