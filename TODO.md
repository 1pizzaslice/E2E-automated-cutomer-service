...
Close Milestone 15, then start Milestone 16 - Real Authentication And Policy Lifecycle (full checklists below):

> Milestone 15 is code-complete. To close it (needs the user-owned API keys): (1) put `ANTHROPIC_API_KEY` (and `OPENAI_API_KEY` for real embeddings / the second-provider proof) in the environment; (2) run the live gate `SUPPORT_LLM_PROVIDER=anthropic SUPPORT_LLM_MODEL=claude-opus-4-8 PYTHONPATH=ai uv run --frozen --project ai --extra llm python -m evals.live_runner`, triage any regressions (prompt-file bumps per SOPS §11.1), and record pass rates in the checklist; (3) drive the real-Claude e2e (sidecar with `SUPPORT_LLM_PROVIDER=anthropic`, e.g. via `pnpm --filter @support/workers test:e2e:service` with the provider env set, or the Compose `ai-service`) and confirm a citation-grounded Claude draft lands in an approval; (4) tick the two blocked checklist items + acceptance criteria. Then start Milestone 16: hosted-IdP JWKS verification in `readAuthContext`, tenant-membership enforcement, machine-token hardening, policy lifecycle endpoints emitting the reserved `policy.*` audit actions, and `retention_policy` on the tenant contract.

Milestone 15 follow-ups to fold into later work (not blockers): the live eval pass rates and the real-Claude e2e drive are the milestone's own closing items (blocked on keys, see above), not follow-ups; Milestone 18's hardened Compose must inject `ANTHROPIC_API_KEY`/`OPENAI_API_KEY` as real secrets (the Compose ai-service currently passes them through from the host env); the workers' keyword triage still seeds initial topic/priority before the AI graph — replacing it with a classification read from the persisted run is deferred to the reopened-ticket work (Milestone 22, ADR-0023 follow-up); the retrieval similarity floor defaults to 0 — set `SUPPORT_KB_MIN_SIMILARITY` (suggested 0.25 for `text-embedding-3-small`) when real embeddings go live (Milestone 19); the scripted provider cannot judge real draft quality — the LLM-graded rubric is Milestone 21; and `evals.live_runner` prints per-suite reports but does not yet persist them — consider writing run artifacts when Milestone 21's expansion lands.
...
- [ ] Run the golden dataset + injection suites against the pilot default model (live, opt-in command) and record pass rates; hard-fail gates must hold; triage regressions before completing. (Command ready: `python -m evals.live_runner`; BLOCKED on the user-owned Anthropic API key — the only remaining Milestone 15 work.)
...
- [ ] A ticket produces a real Claude-drafted, citation-grounded response end to end on local infra. (BLOCKED on the Anthropic API key; drive via the sidecar e2e with `SUPPORT_LLM_PROVIDER=anthropic`.)
...
- [ ] Hard-fail safety gates hold against the real model: zero unsafe auto-send, zero cross-tenant leaks, injection pass rate 1.0. (Gates hold under `scripted`; the real-model run is BLOCKED on the Anthropic API key.)
