...
Start Milestone 18 - Staging Environment On Hardened Compose (full checklist below; Phase 2, ADR-0020; user-owned inputs: the VM, staging/production DNS records, the alert notification channel):

> Build production Dockerfiles for the API, worker, and AI sidecar; a hardened Compose profile under `infra/production/` (restart policies, resource limits, internal-only networks, no dev mounts); a TLS reverse proxy terminating only the API + webhooks; Prometheus/Grafana/Alertmanager provisioned from `infra/observability/` (the Milestone 17 job alert rules included); secrets as env files per the SecretResolver contract (`SUPPORT_AUTH_ISSUER`/`SUPPORT_AUTH_AUDIENCE`, `ANTHROPIC_API_KEY`/`OPENAI_API_KEY`, machine tokens — and either a shared `RAW_PAYLOAD_STORE_DIR` volume for API+worker or the S3/MinIO store + sweeper behind the same ports); nightly PostgreSQL backups with a tested restore drill; and a CI deploy workflow with health-gated rollback. Then apply migrations + pilot seed on staging, run the RLS smoke check, and run the live integration suites once against the staging database (SOPS §19).
