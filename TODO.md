...
- Default stack: TypeScript API/workers, Python AI runtime, Temporal, an in-repo LangGraph-**style** graph engine (`ai/runtime/graph.py` — the LangGraph library is NOT a dependency; ADR-0005 amended, ADR-0016, ADR-0023), PostgreSQL, pgvector, Redis, NATS JetStream, OpenTelemetry.
...
Milestones 18, 20, and 23 are code-complete (see their checklists below and the "Last Session Summary"). With the reviewer console (Milestone 23) done, the two remaining milestones both wait on user-owned inputs that do not yet exist: **Milestone 19 - Live Providers And Go-Live Rehearsal** needs a provisioned staging VM + DNS, a Mailgun account + verified pilot domain, and Meta Business verification; **Milestone 21 - Eval Expansion And Shadow Replay** needs the pilot client's sanitized historical tickets + KB for the shadow-replay half.

**Milestone 19 Slice 0 (provider ingress adapters) landed 2026-07-12 (PR #41).** The provider-shape layer between Mailgun's/Meta's real HTTP posts and the channel adapters had never been written, so M19's checklist could not have passed: the API parsed `application/json` only (Mailgun posts form/multipart → 415), signature verification read Mailgun's _event-webhook_ envelope instead of the flat fields its inbound routes send (→ 403), no Mailgun→neutral field mapping existed (→ 400), and no `GET` route existed so the WhatsApp webhook **could not be registered at all**. All fixed and regression-tested; see the Milestone 19 section. **Carry the caveat:** these were built against the providers' _documented_ shapes, not _observed_ ones — M19 now opens by capturing a real Mailgun payload + Meta handshake and diffing them against the fixtures.

What a new session can do now, in priority order:

1. **Milestone 21 - Eval Expansion And Shadow Replay** is the most-unblocked remaining work: the golden-dataset expansion to the TEST_STRATEGY §4 counts, the LLM-graded draft-quality rubric (soft gates), and the injection re-run are all code and need no user input. Only the shadow-replay harness + report (a checklist item and one acceptance criterion) waits on the pilot client's historical tickets, so Milestone 21 can be advanced ~two-thirds now and closed when that data arrives.
2. **Milestone 19** once the VM + Mailgun exist: stand up staging on the VM from the Milestone 18 artifacts (follow `infra/production/README.md` — fill `.env` + `env/*.env` with Clerk **production** issuer/audience, Anthropic/OpenAI keys, Slack webhook, strong datastore passwords, and the channel secrets `PILOT_MAILGUN_SIGNING_KEY`/`PILOT_MAILGUN_API_KEY` + the `PILOT_WHATSAPP_*` trio, split across `api.env`/`worker.env`; **set the `CONSOLE_*` repo variables before tagging** — the console bundle is baked at CI build time, not on the VM; migrate + seed; RLS smoke; SOPS §19), then capture a real Mailgun delivery and diff it against the Slice 0 fixtures, configure the inbound route → `POST /v1/webhooks/email/mailgun`, confirm signature verification (good + bad-signature 403), verify an approved draft round-trips to a real mailbox threaded via RFC 5322 headers, run the full staging rehearsal and the load/chaos smoke, and wire WhatsApp Cloud API when Meta verification clears (the `GET` handshake route and `channels.config.verify_token_ref` are in place).
3. **Milestone 22** carries real code, not just config, and is easy to under-scope: attachment binary storage, inbound-email HTML sanitization, the **next-response and resolution SLA timers (only `first_response` is ever armed today** — the other due dates are persisted but never fire), and the outbound email subject strategy.

Known, unactioned, and recorded in `docs/HORZ_DESIGN_REVIEW.md` §6 rather than here: citations are carried but **not required** on a draft, and `audit_events` is a plain table with no hash chain (the app role holds `UPDATE`/`DELETE` on it). Both are cheap and both strengthen the pilot's story to a buyer; neither is a go-live blocker.

Milestone 20 follow-ups, resolved/deferred by Milestone 23: the browser-test gap is **closed** (the Playwright `test:e2e:console` walkthrough exists, out of root `pnpm test`), and two backend gaps the console exposed were closed and recorded above (`GET /v1/me`; the `api-client` browser fetch-binding fix). Still deferred (not blockers): the api-client has no conditional-request helper — the Milestone 23 queue polls by re-fetching rather than sending `If-None-Match`/short-circuiting on 304 (the server-side ETag/304 contract is unused by the console for now; add the helper if poll volume warrants); and bespoke typed client methods for less-used admin writes are still add-as-needed.

Milestone 18 follow-ups to fold into later work (not blockers): the S3/MinIO `RawPayloadStore`/`BlobSweeper` behind the existing ports stays deferred — api+worker share the `RAW_PAYLOAD_STORE_DIR` volume for now (ADR-0027(3)); the `pg-backup` scheduler is an in-container sleep-loop (swap for host cron/systemd if the VM prefers it); the offsite backup hook (`BACKUP_UPLOAD_CMD`) and MinIO `mc mirror` target are user-owned; the Caddyfile proxies only the API + webhooks until Milestone 23 adds a `console` route/service; and the collector image is pinned to `0.154.0` (the version the `translation_strategy` exporter config targets) rather than an arbitrary older tag.
...
**2026-07-12 — Milestone 19 Slice 0: provider ingress adapters + documentation honesty (PR #41, merged).** Session framing: a planning/reflection pass comparing this codebase against the sibling `horz/backend/` design-of-record, ahead of the VM/DNS/Mailgun/Meta go-live work. The comparison surfaced something the milestone checklists did not: **the provider-shape ingress layer had never been written**, so M19 could not have passed. Four defects, each verified against source and the providers' current documented contracts:

1. The API registered a body parser for `application/json` only; Mailgun's inbound routes POST `application/x-www-form-urlencoded` (`multipart/form-data` with attachments) → **415**.
2. Mailgun signature verification read the nested `body.signature.{...}` envelope — the shape Mailgun's _event/tracking_ webhooks use. Inbound routes send the triplet as **flat top-level form fields** → **403** on every real delivery.
3. No Mailgun→neutral field mapping existed (`Message-Id` vs `message_id`, `sender`/`from`, Unix-seconds vs ISO-8601) → **400**.
4. No `GET` route existed, so the WhatsApp Cloud webhook **could not be registered at all** — Meta refuses to subscribe a URL that does not echo back `hub.challenge`.

Fixed in four committed slices: `packages/api/src/form-body.ts` (urlencoded + multipart, `rawBody` preserved, attachment bytes drained not stored, truncated files reported _above_ the size limit so they cannot slip the gate), `packages/integrations/src/channels/mailgun-inbound.ts` (the mapper `email-adapter.ts`'s docstring always assumed existed), `GET /v1/webhooks/whatsapp/:provider` (raw challenge as `text/plain`, token via `channels.config.verify_token_ref`, fails closed), a 5-minute Mailgun replay window, and the channel secret config surface (`PILOT_MAILGUN_*`/`PILOT_WHATSAPP_*` were referenced by the pilot seed but present in **no** `.env.example`; the seed also lacked the `sending_domain` whose absence returns `channel_config_invalid` on the first send). Also fixed the CI gap where `support-console` was never built despite Compose referencing it — a clean `docker compose up` on a fresh VM would have failed to pull it.

Why the bugs survived: **the old tests posted a JSON payload Mailgun never sends** — a neutral shape invented to fit the code. Replaced with Mailgun's real field set; the neutral-JSON path keeps coverage via a generic email provider. **Caveat carried forward: these are built against _documented_ shapes, not _observed_ ones — the first live delivery is the real test.**

Documentation honesty, same PR: `docs/HORZ_DESIGN_REVIEW.md` records what horz proposes, what we reject (its one-`agent-core`-instantiated-49× design; its 3-call Haiku→**Opus**→Sonnet-supervisor pipeline, where ours is 2 calls with deterministic Python governance — cheaper _and_ safer), and what it gets right that we defer on purpose (the Company Brain; a signer registry). Separately: **there is no LangGraph in this codebase** — `ai/runtime/graph.py` is an in-repo engine reproducing its API surface, and `langgraph` is in no manifest. The stack list, the M9 title, ADR-0005 and the harness doc all asserted the library; corrected, with an append-only amendment on ADR-0005. Dead `ai/runtime/harness.py` scaffold deleted.

---

**Milestone 23 - Reviewer Console (ADR-0028).** Branch `feat-milestone23-reviewer-console` from `main` (post-#39 merge); `pnpm harness:preflight` green. Session framing: of the three remaining milestones, Milestone 19 stays blocked on the user-owned VM/DNS + Mailgun/Meta accounts and Milestone 21 is only half-unblockable without the pilot client's historical tickets, so **Milestone 23 is the one completable end-to-end without a blocker**. User decisions this session: build Milestone 23 now; UI framework **React + Vite**.

Built as a static Vite + React 19 SPA at `apps/console` over `@support/api-client`, in six committed slices (each typecheck + test + build green before commit):

- **Slice 1 — scaffold + ADR-0028.** Vite/React/vitest toolchain (`@vitejs/plugin-react-swc`, reusing the repo's `@swc/core`; vitest node-default env with a `// @vitest-environment jsdom` docblock so the Milestone 20 contract-proof test is untouched; Playwright wired but out of root `pnpm test`). `pnpm-workspace.yaml` declines Clerk's telemetry-notice postinstall (`allowBuilds: "@clerk/shared": false`). AGENTS.md (root + console) record the framework.
- **Slice 2 — `GET /v1/me` (M20 gap) + auth + shell + nav.** New tenant-optional session endpoint behind a `session:read` permission (granted to every console-facing human role; `permissionsForActor` union; `membershipTenantId` surfaced on the request context so a tenant-bound reviewer learns its home tenant); wired through `api-client`/`openapi.ts`/drift/RBAC-matrix (`@support/api` 214 tests). Console side: an auth-provider seam (`ClerkCredentialsGate` + a dev `TokenCredentialsGate`), `AuthGate` resolving the session via `/v1/me`, the app shell, and permission-gated nav (the console reads effective `permissions`, never replicating `ROLE_PERMISSIONS`).
- **Slice 3 — queue → evidence → decide.** `ApprovalsPage` (summary badge, order/offset/has_more, poll), `EvidencePage` (conversation, AI draft, tool calls, an `ai_runs:read`-gated AI-run card with a trace link), `DecidePanel` (approve/edit/reject/escalate, gated on `approvals:review`; the edit box is seeded with the AI draft and submitted as `approved_payload`).
- **Slice 4 — QA surface.** `QaPage`/`QaReviewPage`/`QaCompleteForm` (`?completed=false` → evidence → complete with 0–5 dimension scores + the closed `QaDefectCategory` taxonomy; write-gated on `qa_reviews:write`); extracted shared `evidence-sections` reused by both evidence views.
- **Slice 5 — test tier.** 13 jsdom component tests (queue/evidence/decide/QA/nav/auth) + a chromium Playwright walkthrough (`test:e2e:console`) driving the real bundle with `/v1/*` mocked at the network boundary. The browser tier caught two real bugs the Node tests could not — both fixed and recorded as Milestone 20 gaps: (a) `@support/api-client` invoked the default global `fetch` unbound → `Illegal invocation` in browsers (bound to `globalThis`); (b) same-origin (`""` base) made the client build `new URL("/v1/me")` which throws (config defaults the base to `window.location.origin`).
- **Slice 6 — infra + docs.** `console` service in the hardened Compose (`console.Dockerfile` → static bundle in a tiny Caddy; edge Caddy proxies non-API paths to `console:80`; `.env.example` `CONSOLE_*` build args); `docker compose config` validates. Docs: SOPS §4/§10/§19, TEST_STRATEGY, PROJECT_HISTORY, README, this file.

Not done (VM-deferred, exactly as Milestones 18/20 deferred their VM steps): the actual production bring-up of the `console` service + public-domain TLS (SOPS §19; needs the VM). Not yet pushed/PR'd — local commits on the branch; `pnpm harness:handoff` before push.

### Previous Session (Milestone 20)

Milestone 20 - Console Enablement API. Branch `feat-milestone20-console-enablement-api` from `main` (post-#38 merge); `pnpm harness:preflight` green. User decisions this session: do Milestone 20 now (Milestone 19 is blocked on user-owned VM/DNS + Mailgun/Meta); pilot targets **email + WhatsApp** (so Meta Business verification should start now, weeks of lead time); approvals open-counts via a **dedicated `GET /v1/approvals/summary` endpoint**; freshness via **ETag + `If-None-Match` → 304**.

Built across four committed slices (each typecheck + test green before commit):

- **Slice A — HTTP hardening.** `packages/api/src/cors.ts` (`@fastify/cors` behind `SUPPORT_CORS_ALLOWED_ORIGINS`, exact-origin, wildcard rejected, preflight before auth) and `rate-limit.ts` (`@fastify/rate-limit` keyed per authenticated principal, `ioredis` store when `REDIS_URL` set, health/webhooks exempt, opt-in via `SUPPORT_RATE_LIMIT_ENABLED`). Routes now register inside a child plugin in `app.ts` so the deferred CORS/rate-limit `onRequest` hooks actually cover them; the framework 429 is surfaced as the taxonomy's `RATE_LIMITED` in `errors.ts`. `apps/*` added to the workspace. `http-hardening.test.ts` (11 tests).
- **Slice B — read surfaces.** `GET /v1/approvals/{id}/evidence` composite + `GET /v1/approvals/summary` (per-status counts via `approvalStatusCountsQuery`); approval/ticket queue ergonomics (`order`/`offset`/`has_more` via +1 over-fetch); `GET /v1/tickets/{id}/events` over the existing `ticketEventsForTicketQuery` + `updated_since`; ETag/304 via `respondWithEtag`. New shared-schemas (`ApprovalEvidence`, `ApprovalSummary`, `TicketEvent` list, `ListSortOrder`, page `offset`/`has_more`), repository `offset`/`order`/`updatedSince`, RBAC catalog entries, `console-enablement.test.ts` (10 tests).
- **Slice C — client + drift.** `packages/api-client` (`SupportApiClient` typed from `@support/shared-schemas`, `ApiClientError`, `API_ROUTES` manifest); `route-drift.test.ts` (registered == OpenAPI both directions, client-facing == `API_ROUTES`); OpenAPI completeness (0 stale paths; the 4 gaps closed — 3 new endpoints + `/openapi.json` — plus offset/order/updated_since params, ETag headers, `NotModified`); `BACKEND_SPEC §17` `x-user-id` text corrected. `api-client` 4 tests.
- **Slice D — console slice + CI + docs.** `apps/console` (`review-flow.ts` `runReviewFlow` over the client only; `review-flow.test.ts` drives login→queue→evidence→decide against the real Fastify app through an `inject`-backed transport — approve + edit) and `apps/console/AGENTS.md`; `ci.yml` restructured to gate uv/Python + NATS + integration behind a `dorny/paths-filter` `heavy` step inside the one required job (JS lint/typecheck/test/build always run); this file, BACKEND_SPEC, TEST_STRATEGY updated.

Not done (deferred to Milestone 23, not blockers): the console UI itself, a client conditional-request helper, and the Playwright `test:e2e:console` tier. No new ADR was required (Milestone 20 is additive within ADR-0026). Not yet pushed/PR'd — local commits on the branch; `pnpm harness:handoff` before push.

### Previous Session (Milestone 18)

Milestone 18 - Staging Environment On Hardened Compose (ADR-0027). Branch `feat-milestone18-staging-hardened-compose` from `main` (post-#37 merge); `pnpm harness:preflight` green. User decisions that session: build + validate locally (no VM this session), Caddy for TLS, Slack webhook for alerts.

- **Production Dockerfiles** (`infra/production/{api,worker,ai}.Dockerfile`). API/worker are multi-stage: builder does a frozen `pnpm install` then `pnpm --filter <pkg> deploy --prod --legacy /app` (pruned isolated closure); runtime is `node:24-bookworm-slim`, non-root, `CMD ["tsx","src/…"]`. Chosen because the monorepo consumes packages as TS source (`exports` → `./src/index.ts`), so there is no compiled-JS path — `tsx` was promoted to a **production** dependency of `@support/api` + `@support/workers` (lockfile updated) so `--prod` keeps it. The AI image hardens `ai/Dockerfile` (uv-locked, non-root `appuser`, pinnable `--workers`). All three build (api 329MB, worker 541MB, ai 433MB) and were smoke-checked (server.ts/migrate.ts/migrations/ present, tsx runs, non-root).
- **Hardened Compose** (`infra/production/docker-compose.yml`, 15 services): app (api/worker/ai) + postgres/redis/nats/temporal(+ui)/minio/collector + caddy + prometheus/grafana/alertmanager + pg-backup. `restart: unless-stopped`, per-service CPU/mem limits, one internal `backend` bridge, no dev mounts. Only Caddy publishes `0.0.0.0:80/443`; operator UIs bind `127.0.0.1` (SSH tunnel); datastores have no host ports. api+worker share a `raw-payloads` volume at `RAW_PAYLOAD_STORE_DIR=/data/raw-payloads` (M17 follow-up, "share the dir" branch — S3/MinIO sweeper deferred). `migrate`/`seed` are `--profile setup` one-shots on the API image. Shared machine tokens + datastore passwords single-sourced in `.env`; per-service external keys in `env/<service>.env`.
- **Caddy** (`caddy/Caddyfile`): TLS terminates `/v1/*` (incl. webhooks) + `/health`; `/internal/*` refused at the edge; local uses the internal cert (`STAGING_DOMAIN=localhost`), the VM uses Let's Encrypt.
- **Monitoring**: `monitoring/prometheus.yml` scrapes `otel-collector:8889` + loads `infra/observability/alerts.yaml`; Grafana provisioning (`monitoring/grafana/provisioning/**`) wires a default Prometheus datasource + the `infra/observability/dashboards/` dashboard; `monitoring/alertmanager.yml` routes to a Slack receiver via `slack_api_url_file`, rendered from `$SLACK_WEBHOOK_URL` by the container entrypoint (empty = no delivery, config-ready).
- **Backups**: `backup/pg-backup.sh` (nightly custom-format `pg_dump`, sha256, retention prune, pluggable `BACKUP_UPLOAD_CMD` offsite hook, `RUN_ONCE` for drills) as the `pg-backup` service; `backup/pg-restore-drill.sh` restores the latest dump to a scratch DB and verifies it.
- **CI deploy** (`.github/workflows/deploy.yml` + `deploy.sh`/`rollback.sh`): matrix build+push api/worker/ai to GHCR on a `v*` tag; SSH deploy gated behind `DEPLOY_ENABLED=true` + `DEPLOY_SSH_*`; `deploy.sh` records the current tag, pins the new one, migrates, rolls, health-gates, auto-rolls-back; `rollback.sh` restores the previous tag; `DEPLOY_SKIP_PULL=1` for local/air-gapped drills.
- **Local validation (full stack, then torn down)**: `docker compose config` valid; all 15 services up/healthy; migrations 0001-0007 + pilot seed applied; RLS smoke passed (unset tenant → raise; cross-tenant read → 0; cross-tenant insert → WITH CHECK rejection); worker connected to Temporal via `sidecar:http://ai-service:8090` and bootstrapped 2 per-tenant schedules; Caddy `/health` 200 over TLS, `/v1/*` 401, `/internal/*` 404; Prometheus 2 targets up + 9 `Support*` rules; Grafana datasource + `Support Platform Overview` dashboard; Alertmanager synthetic critical alert routed to `slack-notifications`; backup→restore drill PASS (7 migrations + tenant); `deploy.sh relB`→`rollback.sh` flipped the running image relA→relB→relA through the health gate; live DB integration suite 19/19 PASS against the production-compose DB (via a temporary port override, removed after). Fixed en route: pinned the collector to `0.154.0` (0.121 rejected the config's `translation_strategy`), and the Temporal healthcheck to target `temporal:7233` (auto-setup binds the container IP, not loopback).
- **Docs**: `infra/production/README.md` (deploy runbook), SOPS §19 refined (M18 mechanics + migration range 0001-0007 + rollback tooling), ADR-0027, this file.
- **Not done (needs the user-owned VM/DNS/Slack, per SOPS §19):** deploying onto the real VM, public-domain TLS, a synthetic alert actually delivered to Slack, and the full live suite over the network. All are one command away from the artifacts above.

### Previous Session (Milestone 17)

- Created feature branch `feat-milestone17-scheduled-jobs-retention` from `main` (post-#35 merge) and ran `pnpm harness:preflight`.
- Completed Milestone 17 - Scheduled Jobs And Retention Execution in one coherent branch (all 7 checklist items + 2 acceptance criteria). Contract decisions recorded in ADR-0025 (per-tenant Temporal Schedules on the lifecycle worker/task queue, fatal create-if-missing bootstrap from the entrypoint, sweep-before-clear fail-closed blob deletion, scheme-partitioned attachment purge, in-place AI-run anonymization with an explicit `anonymized_at` marker, `audit_event_days` deliberately unenforced).
- DB: migration `0007_ai_run_anonymization.sql` + schema (`ai_runs.anonymized_at`); repositories replace the placeholder count queries with `expiredAttachmentMessagesQuery`/`clearMessageAttachmentsQuery` (jsonb `attachments` → `[]`) and `expiredAiRunsForAnonymizationQuery`/`anonymizeAiRunsQuery` (`structured_output` → null, `guardrail_results` → `{}`, guarded by `anonymized_at is null`); compiled-SQL tests updated, migrations test lists `0007`.
- Observability: `SupportMetrics` port gains `recordJobRun` (`support.job.executions` + `support.job.duration_ms`; job/outcome/tenant attributes) and `recordRetentionPurge` (`support.retention.purged_items` per class, zero-count suppressed) across noop/recording/OTel implementations; new `retention_sweep_failed` critical-failure mode; `SUPPORT_ATTR.job`; alerts.yaml gains the `support-scheduled-jobs` group (`SupportScheduledJobFailures` on failed runs, `SupportRetentionSweepFailures` on the new mode); dashboard gains job-runs and retention-purge panels.
- `packages/workers/src/blob-sweeper.ts` (new): `BlobSweeper` port + filesystem implementation (deletes `file://` refs only inside `RAW_PAYLOAD_STORE_DIR`, resolved-path containment check blocks traversal, ENOENT = idempotent success, foreign schemes fail closed) + recording test double. `retention.ts` reworked: sweep-before-clear for raw payloads (only swept refs get their DB rows cleared; failures reported per-ref and retried next run), attachment purge (local `file://` object refs swept fail-closed, then metadata cleared; provider refs like `whatsapp-media:` are metadata-only by design — `isLocallyStoredRef`), AI-run anonymization batch, `retention.applied` audit metadata now carries per-class counts + sweep failures + all three cutoffs, result gains `batchLimitHit`; legacy no-sweeper mode keeps the old clear-and-return-refs contract; in-memory store fixtures reshaped (`attachmentRefs`, `aiRuns` with `anonymizedAt`).
- Scheduled job workflows: `workflows/scheduled-jobs-types.ts` (workflow types/ids, schedule-id helpers, retry policy, `SCHEDULED_JOBS_MAX_BATCHES=50`, snake_case activity contracts) + `workflows/scheduled-jobs-workflow.ts` (`qaSamplingJobWorkflow`/`retentionJobWorkflow`: deterministic bounded-batch drain loops with no-progress stops), re-exported from `ticket-lifecycle-workflow.ts` so the existing single `workflowsPath` bundle registers them. `activities/scheduled-jobs-activities.ts` (new): production activities running one batch per call with `job.*` spans, job metrics, retention purge metrics, `retention_sweep_failed` mapping, structured logs; errors rethrow so Temporal retry semantics hold. `worker-runtime.ts` composes DB retention/QA stores + filesystem sweeper + NATS publisher into the activities map (injectable via new overrides; stores closed on shutdown); `temporal-worker.ts` accepts the merged activities type.
- Schedule bootstrap: `job-schedules.ts` (new) — `loadScheduledJobsConfig` (fail-fast aggregate validation; `SUPPORT_JOB_SCHEDULES` enabled/disabled, `SUPPORT_QA_SAMPLING_SCHEDULE_UTC` 02:00, `SUPPORT_RETENTION_SCHEDULE_UTC` 02:30), `ensureTenantJobSchedules` (create-if-missing via `ScheduleAlreadyRunning`, calendar spec + 5m jitter + overlap SKIP + 6h catch-up, `startWorkflow` action with `{tenant_id}` args), `createDatabaseTenantLister` (owner connection, active tenants only), `bootstrapJobSchedules` (production composition; injectable lister/schedule client). `main.ts` runs the bootstrap after runtime start and exits non-zero if it fails (a worker without guaranteed schedules must not run).
- Tests: new `blob-sweeper.test.ts` (8), `job-schedules.test.ts` (9), `activities/scheduled-jobs-activities.test.ts` (7), reworked `retention.test.ts` (14) — all offline; new committed opt-in live suite `scheduled-jobs.integration.test.ts` (`pnpm test:jobs`, `RUN_SCHEDULED_JOBS_LIVE_TESTS=true`): seeds a tenant with a full retention policy + expired/fresh messages (real blobs on disk) + expired/candidate AI runs, boots the production worker runtime on an isolated task queue, double-bootstraps (second run reports both schedules existing), triggers both schedules, awaits the schedule-started workflow results, and asserts refs cleared/blobs deleted/attachments purged/AI run anonymized (metadata retained)/audit metadata counts, QA reviews queued + `support.qa.review_created.v1` read back from JetStream, and no-op re-triggers (no new audits/reviews) — 1/1 PASS this session. Verification: `pnpm typecheck`/`lint`/`format:check`/`test`/`build` green; `pnpm test:integration` (19+42+6), `pnpm test:e2e` 1/1, `pnpm --filter @support/workers test:workflow` 11/11 green under the new composition.
- Docs: BACKEND_SPEC §19.4 (scheduled job workflows) + §22 rewrite (executed classes, sweep semantics, anonymization, unenforced audit class); SOPS §10 scheduling runbook (bootstrap, Temporal UI/CLI operations, verification metrics) + §16 retention execution/runbook; TEST_STRATEGY §3.9 Milestone 17 coverage + §12 job observability coverage; DECISIONS ADR-0025; README (status line, `pnpm test:jobs`, worker bootstrap paragraph); `.env.example` (schedule + sweeper vars); PROJECT_HISTORY (M17 bullet, verification record, next task + follow-ups); this file.

Previous session (Milestone 16):
...
- Milestone 18 (`feat-milestone18-staging-hardened-compose`): `pnpm harness:preflight` passed on the branch.
- Image builds: all three production images build under the legacy builder (`DOCKER_BUILDKIT=0`; the host lacks buildx, CI has it; no BuildKit-only syntax) — api 329MB, worker 541MB, ai 433MB; each runs non-root and the API image carries `@support/db` src + `migrations/` + `tsx` for the migrate/seed one-shots. Lockfile updated (`pnpm install --lockfile-only`) for `tsx` as a prod dep of api+workers.
- `docker compose -f infra/production/docker-compose.yml config` valid (15 services).
- Full local bring-up + teardown of the production-compose stack (all 15 services healthy): migrations 0001-0007 applied via the `migrate` one-shot; pilot seed applied (tenant=1, users=3, tools=6); RLS smoke PASS as `support_app` (unset `app.current_tenant_id` → `must be set` exception; cross-tenant read → 0 rows; cross-tenant INSERT → `new row violates row-level security policy`); worker connected to Temporal via the real sidecar bridge (`sidecar:http://ai-service:8090`) and bootstrapped 2 per-tenant schedules (`support-qa-sampling-ten_pilot`, `support-retention-ten_pilot`), reached `state: RUNNING`.
- Edge: `curl -k https://localhost/health` → 200 (`{"service":"api","status":"ok",...}`) over Caddy's internal cert; `/v1/tenants` → 401 (auth enforced); `/internal/tools/execute` → 404 (refused at the edge). Hardening confirmed: postgres has no host port (`{"5432/tcp":null}`); only Caddy is `0.0.0.0`-published; Grafana/Prometheus/Alertmanager/Temporal-UI/MinIO-console bind `127.0.0.1`.
- Monitoring: Prometheus 2 targets up + 9 `Support*` alert rules loaded; Grafana health ok + Prometheus datasource + `Support Platform Overview` dashboard provisioned; Alertmanager healthy, config loaded with the `slack-notifications` receiver, a POSTed synthetic critical alert routed to it (Slack delivery no-op until `SLACK_WEBHOOK_URL` is set — file rendered from the env secret by the entrypoint).
- Backups/deploy: backup one-shot wrote a checksummed 112K dump; restore drill PASS (checksum OK, 7 migrations + tenant restored to a scratch DB); `deploy.sh relB` health-gated to "healthy on relB" and recorded relA as previous, `rollback.sh` restored relA — running api image flipped relA→relB→relA.
- Live DB integration suite against the production-compose DB (temporary port override, removed after): `@support/db` test:integration 19/19 PASS (repositories + RLS execution).
- Fixes made during validation: collector pinned `0.121.0`→`0.154.0` (0.121 rejects the config's `translation_strategy`); Temporal healthcheck now targets `temporal:7233` (auto-setup binds the container IP, not `127.0.0.1`); pg-backup mounts the whole `backup/` dir so the restore-drill script is present.
- Not run this session: CI on the branch (runs on push; the new `deploy.yml` only builds+pushes on a `v*` tag and the deploy job is gated behind `DEPLOY_ENABLED`); the full live integration suite over the network and any real-VM/DNS/Slack step (all SOPS §19 boxes closed on the VM). No application source changed — the only code-file change is `tsx` added to two package.json dependency lists — so the existing offline/live behavioral suites are unaffected.
...
- ~~QA sampling and retention jobs have no scheduler; retention attachment/AI-run purges are counted-and-reported placeholders (Milestone 17)~~ — closed by Milestone 17: per-tenant daily Temporal Schedules bootstrap from the worker; retention executes every class fail-closed (ADR-0025).
- The staging **deployment artifacts** now exist and are validated locally (Milestone 18/ADR-0027: production Dockerfiles, hardened `infra/production/` Compose, Caddy TLS, Prometheus/Grafana/Alertmanager, backups+restore, CI deploy/rollback). What is still missing is **user-owned**: the VM + staging/production DNS to run the stack on, the production Clerk instance + Anthropic/OpenAI keys + Slack webhook to fill `env/*.env`, and Mailgun/WhatsApp accounts (Milestone 19). Until the VM + accounts exist, the SOPS §19 VM-only boxes (public-domain TLS, real Slack delivery, live suites over the network) and Milestone 19 cannot be closed.
...
- [x] Schedule the QA sampling job per tenant on Temporal Schedules (daily), with idempotent create-if-missing bootstrap from the worker entrypoint. (`support-qa-sampling-{tenant}` at `SUPPORT_QA_SAMPLING_SCHEDULE_UTC` default 02:00 UTC, jitter 5m, overlap SKIP, catch-up 6h; `bootstrapJobSchedules` in `packages/workers/src/job-schedules.ts` runs from `main.ts` on every start, `ScheduleAlreadyRunning` = left untouched, bootstrap failure is fatal, `SUPPORT_JOB_SCHEDULES=disabled` opt-out; `qaSamplingJobWorkflow` drains bounded batches on the lifecycle task queue. ADR-0025.)
- [x] Schedule the retention job per tenant likewise (daily). (`support-retention-{tenant}` at `SUPPORT_RETENTION_SCHEDULE_UTC` default 02:30 UTC; `retentionJobWorkflow` loops `runRetentionJob` batches with no-progress and max-batch stops.)
- [x] Implement the blob sweeper: delete cleared raw-payload refs from object storage (the retention job already returns them). (New `BlobSweeper` port, `packages/workers/src/blob-sweeper.ts`: filesystem implementation matching the API's `file://` raw-payload store, deletes only inside `RAW_PAYLOAD_STORE_DIR`, ENOENT = idempotent success, foreign schemes/outside paths fail closed; retention now sweeps BEFORE clearing so an undeletable blob keeps its row and retries next run. S3/MinIO sweeper lands behind the same port with Milestone 18.)
- [x] Implement the attachment purge and AI-run anonymization retention classes (currently counted-and-reported placeholders; fail-closed semantics preserved). (Attachments: local `file://` object refs swept fail-closed, then metadata cleared to `[]`; provider-side refs (whatsapp-media:, URLs) are metadata-only by design. AI runs: `structured_output` → null, `guardrail_results` → `{}`, `anonymized_at` stamped (migration `0007`) with run metrics/provenance retained; `anonymized_at IS NULL` keeps re-runs idempotent. `audit_event_days` deliberately unenforced — append-only compliance ledger, ADR-0025.)
- [x] Add job observability: per-run metrics + structured logs, and a job-failure alert rule in `infra/observability/alerts.yaml`. (`SupportMetrics.recordJobRun`/`recordRetentionPurge` → `support.job.executions`/`support.job.duration_ms`/`support.retention.purged_items`; `job.qa_sampling`/`job.retention` spans; per-run structured logs; new `retention_sweep_failed` critical-failure mode; alert rules `SupportScheduledJobFailures` + `SupportRetentionSweepFailures`; two new dashboard panels.)
- [x] Live coverage: schedules fire against local Temporal; a retention run clears refs, sweeps blobs, and audits `retention.applied`; QA sampling emits `support.qa.review_created.v1` on schedule; re-runs stay idempotent. (`packages/workers/src/scheduled-jobs.integration.test.ts`, `pnpm test:jobs` — 1/1 PASS live this session: idempotent double bootstrap, real Schedule triggers through the production worker, refs cleared + blobs deleted from disk + attachments purged + AI run anonymized + `retention.applied` audited with per-class counts, QA reviews queued + envelope read back from JetStream, re-triggers no-op.)
- [x] Update docs (SOPS §10/§16 scheduling runbooks, this file). (Also BACKEND_SPEC §19.4 + §22 rewrite, TEST_STRATEGY §3.9/§12, DECISIONS ADR-0025, README, `.env.example`, PROJECT_HISTORY.)

Acceptance criteria:

- [x] Both jobs run on cadence with no human action, and failures alert. (Daily Temporal Schedules bootstrapped from the worker entrypoint; failed runs alert via `SupportScheduledJobFailures`, unswept blobs via `SupportRetentionSweepFailures`.)
- [x] Retention purges raw payloads, attachments, and AI-run PII per tenant policy and audits every application. (Proven live end to end by `pnpm test:jobs`; fail-closed on missing policy and on sweep failures.)
...
Status: COMPLETE (code + config, ADR-0027) on `feat-milestone18-staging-hardened-compose`. Everything buildable is built and was validated by a full local bring-up of the production-compose stack; the three acceptance items that inherently require the real VM/DNS/Slack (public-domain TLS, a synthetic alert actually reaching Slack, live suites against the VM's DB over the network) are wired one-command-away and are closed on the VM per SOPS §19 (user-owned inputs).

Checklist:

- [x] Production Dockerfiles for the API, worker, and AI sidecar (`infra/production/{api,worker,ai}.Dockerfile`; multi-stage; pruned `pnpm deploy --prod` images running `tsx` on source — ADR-0027(1); uv-locked Python image; all three build, run non-root: api 329MB, worker 541MB, ai 433MB).
- [x] A production Compose profile under `infra/production/docker-compose.yml`: app + PostgreSQL/Temporal/NATS/Redis/MinIO/collector with restart policies, per-service CPU/memory limits, a single internal `backend` bridge network, no dev mounts, no exposed dev ports (validated: postgres has no host port; only Caddy is `0.0.0.0`-published; operator UIs bind `127.0.0.1`). Migrate/seed are profile-gated one-shots on the API image.
- [x] TLS reverse proxy (Caddy) terminating only `/v1/*` + webhooks + `/health`; `/internal/*` refused at the edge; Temporal UI/Grafana/Prometheus/Alertmanager/MinIO console `127.0.0.1`-bound for SSH-tunnel access (validated: 200 over TLS, `/internal/*` → 404, `/v1/*` → 401).
- [x] Prometheus + Grafana + Alertmanager provisioned from `infra/observability/` (validated: Prometheus scrapes the collector + loads all 9 `Support*` alert rules; Grafana auto-provisions the datasource + `Support Platform Overview` dashboard; Alertmanager routes a synthetic critical alert to the Slack receiver). Slack delivery activates when `SLACK_WEBHOOK_URL` is set (rendered to a file the config reads).
- [x] Secrets as git-ignored env files following the SecretResolver contract (`.env` for shared/interpolation incl. the machine tokens that must match across api/worker/ai; `env/<service>.env` per external provider/IdP key), documented per service with committed `.example` templates.
- [x] Automated nightly PostgreSQL backups (`pg-backup` scheduling-loop service: checksummed custom-format `pg_dump`, retention pruning, pluggable offsite `BACKUP_UPLOAD_CMD` hook) with a tested restore drill (`pg-restore-drill.sh`; validated PASS: checksum OK, 7 migrations + tenant restored to a scratch DB); MinIO backup story documented in the README.
- [x] CI deploy workflow (`.github/workflows/deploy.yml`): build + push the three images to GHCR on a `v*` tag, SSH-deploy via `deploy.sh` with a health gate and auto-rollback, one-command `rollback.sh` to the previous tag (gated behind `DEPLOY_ENABLED=true` + `DEPLOY_SSH_*` so it is safe before the VM exists).
- [x] Apply migrate + seed and run the RLS smoke check — done locally on the production-compose DB (migrations 0001-0007, pilot seed, RLS: unset-tenant raises / cross-tenant read = 0 / cross-tenant insert rejected). On the real VM this is a SOPS §19 box.
- [~] Run the live integration suites against the staging database — the live DB suite (repositories + RLS execution, 19 tests) passed against the production-compose DB; the full suite (needs NATS + the app over the network) is a VM-time SOPS §19 step (hardening keeps those ports internal).
- [x] Update docs (`infra/production/README.md` deploy runbook, SOPS §19 refinements, ADR-0027, this file).

Acceptance criteria:

- [x] Staging serves the API over TLS with the worker and sidecar running as supervised services — validated locally (Caddy TLS 200; worker + ai-service healthy; worker drives the real sidecar bridge and bootstraps the per-tenant job schedules). The public-domain instance is user-owned (SOPS §19).
- [~] Grafana shows live dashboards; a synthetic critical failure fires an alert to the real channel — Grafana dashboards + the full Prometheus→Alertmanager→Slack-receiver routing validated locally with a synthetic alert; the final hop to a real Slack channel needs the user's `SLACK_WEBHOOK_URL` (user-owned).
- [x] Backup restore and deploy rollback have each been exercised successfully at least once — restore drill PASS; `deploy.sh`→`rollback.sh` flipped the running image relA→relB→relA through the health gate.
...
Status: COMPLETE on `feat-milestone20-console-enablement-api`. All backend read/enablement surfaces, the typed client, the drift test, and the console contract-proof slice are in and green (typecheck + lint + build clean; api 210 tests, api-client 4, console 2, plus the new console-enablement/http-hardening suites). The console UI itself remains Milestone 23; M20 delivered only the contract proof.

Checklist:

- [x] CORS support with an env-driven origin allowlist (off by default). (`packages/api/src/cors.ts`: `@fastify/cors` behind `SUPPORT_CORS_ALLOWED_ORIGINS`; exact-origin match, wildcard rejected, preflight answered before auth.)
- [x] Approval queue ergonomics on `GET /v1/approvals`: pending/status filters, age/created ordering, pagination hardening, and a cheap open-counts summary. (`order=created_asc|created_desc`, `offset`, `has_more` via +1 over-fetch; new `GET /v1/approvals/summary` → per-status counts + total via one grouped aggregate.)
- [x] `GET /v1/approvals/{approval_id}/evidence`: the reviewer composite, mirroring the QA evidence read. (`ApprovalEvidenceResponse` = approval + ticket + conversation + messages + ai_run + tool_calls + prior_approvals; `services.approvals.evidence`.)
- [x] `GET /v1/tickets` ergonomics and a `ticket_events` read API (the Milestone 13 follow-up). (`order`/`offset`/`updated_since` on the list; new `GET /v1/tickets/{ticket_id}/events` over the existing `ticketEventsForTicketQuery`.)
- [x] Verify reviewer identity comes from the verified token; fix the stale `BACKEND_SPEC.md` §17 `x-user-id` text. (Confirmed `context.actor.userId`; §17 text corrected to the verified-token subject.)
- [x] Queue freshness contract for polling clients documented in OpenAPI; SSE deferred. (ETag + `If-None-Match` → 304 on the list endpoints via `respondWithEtag`; `updated_since` filter on tickets; documented with ETag headers + `NotModified` responses in `openapi.ts`.)
- [x] Basic rate limiting on authenticated endpoints backed by Redis. (`packages/api/src/rate-limit.ts`: `@fastify/rate-limit` keyed per authenticated principal (IP fallback), Redis store via `ioredis` when `REDIS_URL` is set; health probes + webhooks exempt; off unless `SUPPORT_RATE_LIMIT_ENABLED=true`; framework 429 surfaced as the taxonomy's `RATE_LIMITED`.)
- [x] `packages/api-client`: a typed client over `/v1/*`, typed from `@support/shared-schemas`, consumed by `apps/console` at `workspace:*`, publishable-shaped. (`SupportApiClient` + `ApiClientError` + the `API_ROUTES` manifest.)
- [x] Route↔spec drift test reusing the `onRoute` collector, asserting every registered route appears in `buildOpenApiDocument()` and in `packages/api-client`. (`packages/api/src/route-drift.test.ts`: registered == OpenAPI (both directions) and client-facing registered == `API_ROUTES`; normalizes `:param`↔`{param}`; excludes `/internal/*` + `/v1/webhooks/*` from the client check only.)
- [x] OpenAPI completeness pass, with a documented verification command. (Spec had 0 stale paths; the 4 gaps — the 3 new endpoints + `/openapi.json` — are closed. Verification = the drift test, run by `pnpm -r test`.)
- [x] `pnpm-workspace.yaml` gains an `apps/*` glob; `apps/console/AGENTS.md` scopes the console reading path to the OpenAPI doc + `packages/api-client`.
- [x] CI: gate the heavy steps behind a `dorny/paths-filter` step **inside** the existing job. (`.github/workflows/ci.yml`: JS lint/typecheck/test/build always run; uv/Python, NATS, and live integration gated on a `heavy` filter so a frontend/docs-only change keeps the required check green without running them.)
- [x] Console vertical slice at `apps/console`: login → approval queue → one evidence view → decide. (`apps/console/src/review-flow.ts` `runReviewFlow` over the client only; `review-flow.test.ts` drives it against the real Fastify app through an `inject`-backed transport — approve and edit paths.)
- [x] Update docs (BACKEND_SPEC console API section, TEST_STRATEGY frontend tier, this file).

Acceptance criteria:

- [x] The vertical slice implements login → queue → evidence → decide purely from the OpenAPI doc and `packages/api-client`, and every backend gap it exposed is closed inside this milestone. (No gap surfaced beyond the planned surfaces; the slice imports nothing but `@support/api-client`.)
- [x] The drift test fails when a route is added to `routes.ts` without a corresponding `openapi.ts` entry. (Proven en route: the 3 new endpoints failed the drift test until their `openapi.ts` paths were added.)

Milestone 20 follow-ups to fold into Milestone 23 (not blockers): the api-client has no conditional-request helper yet — the ETag/304 freshness contract is server-side, and a polling wrapper that sends `If-None-Match` and short-circuits on 304 lands with the console's queue view; the client covers `/v1/*` reads + the console write flow with typed methods and the full `API_ROUTES` manifest, but bespoke typed methods for the less-used admin writes can be added when a screen needs them; and the console vertical slice is a Node/vitest contract proof, not a browser test — the Playwright `test:e2e:console` tier is Milestone 23 (kept out of root `pnpm test`).
...
Status: CODE-COMPLETE on `feat-milestone23-reviewer-console` (ADR-0028). A static Vite + React 19 SPA at `apps/console` over `@support/api-client`, built in six committed slices. Two backend gaps it exposed were closed and recorded as Milestone 20 gaps (a `GET /v1/me` session endpoint; an `@support/api-client` fetch-binding fix). The only VM-deferred item is the actual production bring-up of the `console` service (SOPS §19), exactly as Milestones 18/20 deferred their VM steps.

Checklist:

- [x] Pick and record the UI framework in an implementation ADR. (ADR-0028: static Vite + React 19 SPA — no BFF; `vite build` participates in `pnpm -r build`. Next.js/Remix rejected as they add a server deployable. AGENTS.md defaults updated.)
- [x] Clerk login sharing the Milestone 16 application. (`ClerkCredentialsGate` behind an auth-provider seam; a `TokenCredentialsGate` dev path keeps the console drivable locally + in Playwright without Clerk. Session resolves via `GET /v1/me`.)
- [x] Approval queue: `GET /v1/approvals` with the Milestone 20 filters/ordering/pagination and polling for freshness. (`ApprovalsPage`: oldest-pending-first, `order`/`offset`/`has_more`, a live `GET /v1/approvals/summary` badge, and a 10s reload poll.)
- [x] Evidence view: `GET /v1/approvals/{id}/evidence` rendering conversation, the AI draft, tool calls, guardrail results, and the AI run's trace link. (`EvidencePage` + shared `evidence-sections`; the AI-run card is gated on `ai_runs:read`; the trace id links out when `VITE_TRACE_URL_TEMPLATE` is set.)
- [x] Decide actions: approve / edit / reject / escalate, with a draft editor keeping `requested_payload` visibly distinct. (`DecidePanel`, gated on `approvals:review`; the edit box is seeded with the AI draft and submitted as `approved_payload`; the original stays in the evidence above.)
- [x] QA surface: `GET /v1/qa-reviews?completed=false` → evidence → `POST /v1/qa-reviews/{id}/complete` with dimension scores and the defect taxonomy. (`QaPage`/`QaReviewPage`/`QaCompleteForm`: 0–5 scores + the closed `QaDefectCategory` taxonomy with severity; write-gated on `qa_reviews:write`.)
- [x] Permission-gated navigation — `qa_reviewer` and `client_viewer` are read-only on approvals; `client_viewer` cannot see AI runs. (Driven off the caller's effective `permissions` from `GET /v1/me` — the console reads them rather than replicating `ROLE_PERMISSIONS`; the API stays the authority.)
- [x] Frontend test tier: component tests in the console package + a Playwright walkthrough behind `test:e2e:console` (out of root `pnpm test`). (13 jsdom component tests; a chromium walkthrough driving the real bundle with `/v1/*` mocked at the network boundary. The browser tier caught two real bugs — see the Milestone 20 gaps below.)
- [x] `console` service in the hardened profile + extend the TLS reverse proxy. (`infra/production/console.Dockerfile` builds the static bundle into a tiny Caddy; the edge Caddy now proxies non-API paths to `console:80`; `docker-compose.yml` + `.env.example` gain the service and `CONSOLE_*` build args. `docker compose config` validates. Actual VM bring-up is SOPS §19.)
- [x] Update docs (SOPS §4/§10/§19, TEST_STRATEGY, this file).

Milestone 20 gaps closed inside Milestone 23 (per the acceptance criterion): (1) **`GET /v1/me`** — the console needs the caller's roles/permissions to gate navigation and a tenant-bound reviewer needs its home tenant to scope requests, but the Clerk token carries neither (roles are DB-sourced, ADR-0024). Added a tenant-optional session endpoint behind a new `session:read` permission (every console-facing human role; not `integration_admin`/`internal_service`), wired through `api-client` + `openapi.ts` + the drift + RBAC-matrix tests. (2) **`@support/api-client` fetch binding** — the default global `fetch` was stored as a field and invoked unbound, throwing `Illegal invocation` in browsers (the Milestone 20 contract-proof test always injected a fetch, so it never surfaced). Bound it to `globalThis`.

Acceptance criteria:

- [x] A reviewer completes the SOPS §4 ticket-handling loop end to end in the browser: queue → evidence → edit draft → approve → the reply sends. (`test:e2e:console` chromium walkthrough, green.)
- [x] A `qa_reviewer` completes a QA review end to end; a `client_viewer` is denied the approval-decide and AI-run surfaces in the UI, not merely by the API. (Component tests: the QA scorecard submit for `qa_reviewer`; evidence gating hides the AI-run card and decide actions from `client_viewer`.)
- [x] No backend change is required to ship the console beyond gaps recorded as Milestone 20 gaps. (Two gaps surfaced and were closed + recorded above.)
...
- Milestone 23 - Reviewer Console completed on `feat-milestone23-reviewer-console` (ADR-0028). A static Vite + React 19 SPA at `apps/console` over `@support/api-client`, in six committed slices: (1) the framework scaffold + ADR-0028 (Vite/React/vitest, node-default test env with a jsdom docblock so the M20 contract proof is untouched, Playwright wired out of root `pnpm test`); (2) a tenant-optional `GET /v1/me` session endpoint (new `session:read` permission; `permissionsForActor`; `membershipTenantId` on the request context) wired through api-client/OpenAPI/drift/RBAC-matrix, plus the console auth-provider seam (Clerk + a dev token gate), `AuthGate`, app shell, and permission-gated nav driven off the caller's effective permissions; (3) the queue → evidence → decide loop (summary badge, order/offset/has_more + poll; the AI-run card gated on `ai_runs:read` with a trace link; approve/edit/reject/escalate gated on `approvals:review`, the edit box seeded with the AI draft and submitted as `approved_payload`); (4) the QA surface (`?completed=false` → evidence → complete with 0–5 dimension scores + the closed defect taxonomy, write-gated on `qa_reviews:write`) + extracted shared evidence sections; (5) the test tier — 13 jsdom component tests + a chromium Playwright walkthrough (`test:e2e:console`) driving the real bundle with `/v1/*` mocked; (6) the `console` service in the hardened Compose (its image builds + serves the SPA with history fallback — validated by an actual local image build + probe; edge Caddy proxies non-API paths to `console:80`) + docs.
- Two backend gaps the console exposed were closed and recorded as Milestone 20 gaps (per the M23 acceptance criterion): `GET /v1/me` (the token carries no roles/home-tenant; the console needs both for nav gating + tenant scoping), and an `@support/api-client` browser fetch-binding bug (`Illegal invocation` — the default global `fetch` was invoked unbound; the M20 Node contract-proof test always injected a fetch, so the browser tier is what caught it). No new ADR beyond ADR-0028 (M23 is additive within ADR-0026).
- Verified: `pnpm -r typecheck`/`lint`/`format:check` clean; test counts — `@support/console` 13 (jsdom component) + 1 chromium `test:e2e:console` walkthrough; `@support/api` 214 (incl. drift + RBAC matrix + `/v1/me` shape); `@support/api-client` 4. `docker compose config` validates the console service; the `console` image built (61 MB) and served `/` + the `/approvals` SPA fallback (both 200). VM-deferred (as Milestones 18/20 deferred theirs): the actual `console` bring-up on the VM + public-domain TLS (SOPS §19). Docs: ADR-0028, AGENTS.md (root + `apps/console`), SOPS §4/§10/§19, TEST_STRATEGY, PROJECT_HISTORY, README, `.env.example`, this file.

### 2026-07-10

- Milestone 20 - Console Enablement API completed on `feat-milestone20-console-enablement-api` (additive within ADR-0026; no new ADR). Delivered everything `apps/console` needs without backend rework, in four committed slices. Slice A: env-driven CORS allowlist + per-principal Redis rate limiting (`@fastify/cors`/`@fastify/rate-limit`/`ioredis`), routes moved into a child plugin so the deferred hooks apply, framework 429 → `RATE_LIMITED` taxonomy code, `apps/*` workspace glob. Slice B: the approval evidence composite, `GET /v1/approvals/summary`, `ticket_events` read API, approval/ticket queue ergonomics (order/offset/has_more) + `updated_since`, and ETag/`If-None-Match`→304 freshness — with new shared-schemas, repository filters, and RBAC-catalog entries. Slice C: `packages/api-client` (typed `SupportApiClient` from `@support/shared-schemas` + `API_ROUTES` manifest), a route↔OpenAPI↔client drift test, an OpenAPI completeness pass (0 stale paths; 4 gaps closed), and the stale `BACKEND_SPEC §17` `x-user-id` fix. Slice D: the `apps/console` contract-proof slice (`runReviewFlow` login→queue→evidence→decide over the client, driven against the real Fastify app via an `inject` transport), `apps/console/AGENTS.md`, and CI restructured to gate uv/Python + NATS + integration behind a `dorny/paths-filter` step inside the single required job.
- Validated locally: `pnpm -r typecheck`, `pnpm -r lint`, `pnpm build`, `pnpm format:check` clean; test counts — api 210 (incl. drift + http-hardening + console-enablement), api-client 4, console 2. OpenAPI now documents all 56 registered routes (drift test green both directions). The console UI, an api-client conditional-request helper, and the Playwright `test:e2e:console` tier are deferred to Milestone 23. User inputs confirmed this session: proceed with M20 (M19 blocked on VM/DNS + Mailgun/Meta); pilot targets email + WhatsApp (Meta verification is a now-start user task).
- Milestone 18 - Staging Environment On Hardened Compose completed (code + config) on `feat-milestone18-staging-hardened-compose`; contract decisions recorded in ADR-0027. Built the production deployment artifacts under `infra/production/`: multi-stage Dockerfiles for api/worker (pruned `pnpm deploy --prod` images running `tsx` on source — `tsx` promoted to a prod dependency of `@support/api`+`@support/workers`) and a hardened AI sidecar image; a 15-service hardened Compose profile (only Caddy public, operator UIs `127.0.0.1`-bound, one internal bridge, restart policies + resource limits, shared raw-payload volume, profile-gated migrate/seed one-shots); a Caddy TLS reverse proxy (`/v1/*`+webhooks+`/health` only, `/internal/*` refused); Prometheus+Grafana+Alertmanager provisioned from `infra/observability/` with a Slack receiver; git-ignored per-service env-file secrets with committed templates; a nightly `pg_dump` backup service + a tested restore drill; and a `DEPLOY_ENABLED`-gated CI deploy workflow (`.github/workflows/deploy.yml`) with a health-gated `deploy.sh`/`rollback.sh`.
- Validated by a full local bring-up + teardown of the whole stack: all 15 services healthy; migrations 0001-0007 + pilot seed; RLS smoke (unset-tenant raise / cross-tenant read 0 / cross-tenant insert rejected); worker→sidecar bridge with per-tenant job schedules bootstrapped; Caddy TLS 200, `/internal/*` 404, `/v1/*` 401; Prometheus 9 alert rules; Grafana dashboard; Alertmanager synthetic-alert routing; backup→restore PASS; deploy→rollback image flip; 19-test live DB suite against the production-compose DB. Two version pins fixed during validation (collector `0.154.0`; Temporal healthcheck → `temporal:7233`).
- Not done (user-owned, per SOPS §19): the actual VM/DNS deploy, public-domain TLS, real Slack delivery, and the full live suite over the network. Docs: `infra/production/README.md`, SOPS §19 refinements, ADR-0027, and this file. Next code milestone that is unblocked without the VM is Milestone 20 (Console Enablement API); Milestone 19 waits on the VM + Mailgun.
