...
Start Milestone 14 - AI Runtime Service Bridge (full checklist below; platform decision in ADR-0020):

> Stand the Python AI runtime up as the FastAPI sidecar (`ai/service/`, `POST /internal/ai/run` with an internal bearer token), implement the production TypeScript `runAiGraph` activity calling it over HTTP with explicit timeout and retryable-vs-permanent classification (replacing the Milestone 13 deterministic stand-in `createDeterministicRunAiGraph` behind the unchanged `createPersistedRunAiGraph` seam — sidecar unavailability degrades to a structured `failed` result routed to human), expose `POST /internal/tools/execute` speaking the Milestone 8 envelope plus the Python `ToolExecutor`/`RetrievalPort` network adapters, feed `PolicyContext` from `createDatabaseAutomationPolicyStore`, propagate `correlation_id`/`trace_id`, add the sidecar to Compose, and prove service-path runs identical to in-process runs via the eval suite.
...
- Created feature branch `feat-milestone13-worker-entrypoint-ticket-persistence` from `main` and ran `pnpm harness:preflight`.
