...
- Project phase: Milestone 3 API skeleton is complete with tenant/customer/ticket list-create-read-update contracts plus conversation/message/policy/KB document metadata/approval/audit event read-list contracts, ticket audit event list contracts, RBAC checks, and PostgreSQL-backed API integration coverage. Milestone 4 event bus foundation is complete with typed event payload schemas, subject naming, publisher wiring, workflow-ready emit helpers, explicit local NATS JetStream domain/error stream config, worker-side consumer base/idempotency/error handling, and live publish/consume integration coverage. Milestone 5 Temporal workflow foundation is complete with the deterministic ticket workflow shell, activity boundaries, first-response SLA timer breach behavior, an AI graph activity placeholder with success/failure-to-human routing, a `sendOutboundMessage` activity placeholder with deterministic approval-outcome routing (approved/edited send once, rejected does not send, escalated routes to manual handling), explicit activity retry policies, and replay coverage. Milestone 6 channel intake is complete (normalized inbound schema, provider adapters, signature verification, webhook ingress, tenant-scoped persistence, and workflow start/signal wiring). Milestone 7 KB and retrieval is ~half complete: the ingestion vertical (KB document/chunk ingestion contracts, deterministic chunking + embedding pipelines, a pgvector HNSW cosine index, content stored by reference, and tenant-scoped document create/update/ingest endpoints) is done; tenant-scoped retrieval/search and its citation/stale/eval/isolation tests are the pending retrieval half.
- Current milestone: Milestone 7 - KB And Retrieval is ~half complete (the ingestion vertical, in branch `feat-milestone7-kb-ingestion`). Done this session: the ingestion contracts in `packages/shared-schemas` (`KbDocumentCreateRequestSchema`, `KbDocumentUpdateRequestSchema`, `KbChunkResponseSchema`, `KbIngestionResultSchema`); the pure/deterministic `chunkDocument` and `Embedder`/`createDeterministicEmbedder` pipelines in `packages/integrations/src/kb`; the `0003_kb_vector_index.sql` pgvector HNSW cosine index plus tenant-scoped KB write helpers in `packages/db`; and the `packages/api` ingestion vertical (`KbContentStore` port, tenant-scoped `KbIngestionStore` under RLS, `KbIngestionService`) exposed as `POST /v1/kb/documents`, `PATCH /v1/kb/documents/{id}`, and `POST /v1/kb/documents/{id}/ingest` behind a new `kb_documents:write` permission. Verified end to end against a live pgvector database. The remaining six checklist items (tenant-scoped retrieval/search, citation metadata, stale/active exclusion, retrieval eval fixtures, prompt-injection test content, tenant-isolation retrieval tests) are the retrieval half and are the next session's single branch.
...
> Complete Milestone 7 - KB And Retrieval (the retrieval half) in one branch. The ingestion half is done (branch `feat-milestone7-kb-ingestion`): documents can be created, chunked, embedded (`Embedder` port, `vector(1536)`), and stored as active `kb_chunks` with an HNSW cosine index. Now add tenant-scoped retrieval over `kb_chunks`: a `POST /v1/kb/search` endpoint (and/or a KB search service the AI runtime will call) that embeds the query with the same `Embedder`, runs a cosine (`<=>`) nearest-neighbour search filtered to the tenant and `status = 'active'` (excluding stale/inactive/draft), and returns chunk citation IDs plus source/document metadata. Add the `KbChunkResponseSchema`-based retrieval response, a `kb_search`-style permission, retrieval evaluation fixtures, prompt-injection test content (adversarial KB chunks that must not alter retrieval/answers), and tenant-isolation retrieval tests proving retrieval never crosses a tenant boundary. Use the existing deterministic embedder for reproducible retrieval tests; keep the `Embedder` port swappable for a production model. Reuse `activeKbChunksForDocumentQuery` as the pattern for a new tenant+active vector-search query helper.
...
Milestone 7 ingestion follow-ups to fold into the retrieval half or later (not blockers): a live-PostgreSQL KB ingestion integration test (needs a seeded user for `created_by_user_id` FK, or keep it null); a Temporal `KbIngestionWorkflow` driving `load_document`/`chunk_document`/`embed_chunks`/`write_chunks`/`mark_document_active` as activities instead of the synchronous API path; and choosing/documenting a production embedding model behind the `Embedder` port (re-embed if its dimension != 1536).
...
chunking, embedding, pgvector index). The retrieval half (remaining six items) is
the next session's single branch. Note: KB document `status` PATCH already provides
the stale/active mechanism; the remaining "stale document handling" work is
excluding stale/inactive chunks at retrieval time.

Acceptance criteria:

- [ ] Retrieval never crosses tenant boundary. (Ingestion store is tenant-scoped under RLS and unit-tested for cross-tenant isolation; the retrieval query is next.)
- [ ] Results include citation IDs and source metadata. (Chunk `metadata` already carries `document_type`/`source_type`; retrieval must surface chunk/document citation IDs.)
- [ ] Stale/inactive policy docs are excluded from active answers. (Status mechanism exists; retrieval-time exclusion is next.)
