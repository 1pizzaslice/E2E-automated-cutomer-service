...
Start Milestone 16 - Real Authentication And Policy Lifecycle (full checklist below; platform decision in ADR-0020):

> Confirm the IdP (default Clerk; user-owned account setup), implement JWKS-based JWT verification in `readAuthContext` (issuer/audience/expiry, cached key rotation, token subject → `users` row with DB-sourced roles), enforce tenant membership server-side (403 on non-membership), keep webhooks signature-authenticated and health public with the header-injection mode surviving only behind an explicit env flag for tests, add the policy lifecycle endpoints (create/activate/archive with activation immutability, emitting the reserved `policy.created|activated|archived` audit actions so automation-allowlist changes become API-driven), surface `retention_policy` on the tenant contract, and re-run the RBAC matrix under real-token fixtures with the negative suites (expired/forged/wrong-audience → 401 everywhere; non-member tenant → 403; internal endpoints reject user tokens).
