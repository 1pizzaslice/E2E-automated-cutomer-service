...
Start Milestone 15 - Provider-Agnostic Model Layer And Real LLM Default (full checklist below; platform decision in ADR-0020):

> Implement the config-driven Python `ModelProvider` over LangChain's `init_chat_model` behind the existing port (`SUPPORT_LLM_PROVIDER`/`SUPPORT_LLM_MODEL` from env, structured outputs enforced, timeouts/retries, token + latency capture; deps via `uv sync --project ai --extra llm`) with the pilot default Anthropic Claude activated only by explicit config, decide the real-LangGraph swap-or-defer (ADR-0016 seam), add versioned prompt files with stable IDs, implement the production TypeScript `Embedder` factory (env-selected, pilot default OpenAI `text-embedding-3-small`, 1536-dim only) shared by ingestion and retrieval with model-id recording and match enforcement, add the similarity-score floor and max-context cap, capture model/prompt/latency/tokens/cost on `ai_runs` end to end, and run the golden + injection suites against the real model (live, opt-in) with the hard-fail gates holding. The Milestone 14 sidecar is the delivery vehicle: the provider swap happens inside `ai/service/` behind the same `POST /internal/ai/run` contract (`create_app`'s ports factory / config select the model), and `service.eval_parity` plus the eval gates are the regression harness.

Milestone 14 follow-ups to fold into later work (not blockers): the runtime's `approval_package` is dropped at the activity boundary until a consumer exists (revisit for the Milestone 20 console); the sidecar's `ai_runs` provenance constants (`AI_SIDECAR_RUN_PROVENANCE`) should start reading real model metadata from the runtime result when Milestone 15 lands; the `internal_service` machine token is a single shared static secret until Milestone 16's IdP work (rotation = env change + restart); Compose's `ai-service` defaults to local dev tokens — Milestone 18's hardened profile must override them and move the sidecar onto an internal-only network; and the runtime priority vocabulary (`p1`-`p4`) vs platform (`p0`-`p3`) unification is a Milestone 15 item (the real model should emit platform priorities).
...
- The AI runtime runs only in-process with the deterministic model (now the TypeScript `createDeterministicRunAiGraph` stand-in in the worker); no service bridge, no real LLM or embedding providers configured (Milestones 14-15). The deterministic embedder is lexical, so real retrieval quality is unproven until Milestone 15.
