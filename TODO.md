...
Milestone 18 is code-complete (ADR-0027; see the checklist below and the "Last Session Summary"). The next code milestone is **Milestone 19 - Live Providers And Go-Live Rehearsal** (Phase 2; checklist below), but it is **gated on user-owned inputs that do not yet exist**: a provisioned staging VM + DNS (to deploy the Milestone 18 stack onto), a Mailgun account + verified pilot domain, and — for the WhatsApp track — Meta Business verification. Until the VM + Mailgun exist, Milestone 19 cannot start.

What a new session can do now, in priority order:

1. **Stand up staging on the VM** using the Milestone 18 artifacts (once the user provides the VM + DNS): follow `infra/production/README.md` — fill `.env` + `env/*.env` (Clerk **production** issuer/audience, Anthropic/OpenAI keys, Slack webhook, strong datastore passwords), `docker compose -f infra/production/docker-compose.yml up -d`, run the `migrate` + `seed` one-shots, the RLS smoke, and then the SOPS §19 checklist boxes that require the VM (public-domain TLS, a real synthetic alert to Slack, the full live integration suites over the network). Set `DEPLOY_ENABLED=true` + the `DEPLOY_SSH_*` secrets so CI drives deploys on a `v*` tag.
2. **Then Milestone 19** once Mailgun is ready: configure the Mailgun inbound route → `POST /v1/webhooks/email/mailgun`, confirm signature verification (good + bad-signature 403), verify an approved draft round-trips to a real mailbox threaded via RFC 5322 headers, run the full staging rehearsal (real inbound → ticket → real-model draft with citations → approval → delivered → audit/QA), and the load/chaos smoke.
3. **Or work an unblocked code milestone in parallel:** Milestone 20 (Console Enablement API) has no user-owned dependency beyond the shared Clerk app and can proceed now (CORS, approval-queue ergonomics + evidence composite, `ticket_events` read API, Redis rate limiting, `packages/api-client`, the route↔spec drift test). This unblocks Milestone 23 (the console) landing before the Milestone 22 go-live.
...
Milestone 18 - Staging Environment On Hardened Compose (ADR-0027). Branch `feat-milestone18-staging-hardened-compose` from `main` (post-#37 merge); `pnpm harness:preflight` green. User decisions this session: build + validate locally (no VM this session), Caddy for TLS, Slack webhook for alerts.
