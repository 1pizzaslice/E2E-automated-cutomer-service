...
> Begin Milestone 11 - Observability And QA. Add OpenTelemetry tracing and structured logs with trace ids across `packages/api` and `packages/workers` (the Compose stack already ships an OTel collector); add metrics for APIs, workflows, AI runs, tool calls, and approvals; add the QA review data model, QA sampling job, and AI run trace links (persisting `ai_runs` rows here also unblocks the `approvals.ai_run_id`/`messages.ai_run_id` FK linking that Milestone 10 guards around); add basic dashboard definitions and alert definitions for critical failures. Acceptance: a ticket can be traced end to end; QA review can see conversation, evidence, tool calls, AI output, human edits (already stored on approvals by Milestone 10), and the final response; critical failure modes have metrics or alerts.

Milestone 10 follow-ups to fold into later work (not blockers): a production worker entrypoint composing `createTicketLifecyclePersistenceActivities` (+ `createDatabaseTicketLifecyclePersistenceStore` + `createHttpOutboundChannelSender`) with the remaining placeholder activities into `createTicketLifecycleWorker`; a live-PostgreSQL workers integration test for `createDatabaseTicketLifecyclePersistenceStore` (the flow was driven live this session, but not as a committed opt-in test); migrating `messages.send_status`/`sent_by_type` from free text to PostgreSQL enums (contracts already exist in shared-schemas); an outbound email subject strategy (currently null — email threading relies on the RFC 5322 reply headers); and approval expiry handling (`expired` status + return-to-queue, BACKEND_SPEC §12).
