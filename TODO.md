...
Milestones 18 and 20 are code-complete (see their checklists below and the "Last Session Summary"). Two milestones are now unblocked without user-owned inputs; **Milestone 19 - Live Providers And Go-Live Rehearsal** remains **gated on user-owned inputs that do not yet exist**: a provisioned staging VM + DNS (to deploy the Milestone 18 stack onto), a Mailgun account + verified pilot domain, and — for the confirmed WhatsApp track — Meta Business verification (weeks of lead time; start it now). Until the VM + Mailgun exist, Milestone 19 cannot start.

What a new session can do now, in priority order:

1. **Milestone 23 - Reviewer Console** is now unblocked: Milestone 20 delivered `packages/api-client` (typed `SupportApiClient` + `API_ROUTES`), the approval evidence composite, `GET /v1/approvals/summary`, `ticket_events` read API, queue ergonomics + ETag freshness, CORS, and the `apps/console` scaffold with a contract-proof slice (`apps/console/src/review-flow.ts`). M23 picks the UI framework (implementation ADR), builds queue → evidence → decide + the QA surface against the client, and adds a `console` service to the hardened Compose + TLS proxy. Should land before the Milestone 22 go-live.
2. **Milestone 21 - Eval Expansion And Shadow Replay** is partly unblockable now (the golden-dataset expansion, LLM-graded rubric, and injection re-run are code; the shadow-replay harness wants historical tickets + the pilot KB, which are user-owned).
3. **Milestone 19** once the VM + Mailgun exist: stand up staging on the VM from the Milestone 18 artifacts (follow `infra/production/README.md` — fill `.env` + `env/*.env` with Clerk **production** issuer/audience, Anthropic/OpenAI keys, Slack webhook, strong datastore passwords; bring up; migrate + seed; RLS smoke; SOPS §19), then configure the Mailgun inbound route → `POST /v1/webhooks/email/mailgun`, confirm signature verification (good + bad-signature 403), verify an approved draft round-trips to a real mailbox threaded via RFC 5322 headers, run the full staging rehearsal and the load/chaos smoke, and wire WhatsApp Cloud API when Meta verification clears.

Milestone 20 follow-ups to fold into Milestone 23 (not blockers): the api-client has no conditional-request helper yet (the ETag/304 contract is server-side; a polling wrapper that sends `If-None-Match` lands with the console queue view); the client covers `/v1/*` reads + the console write flow with typed methods plus the full `API_ROUTES` manifest — add bespoke typed methods for less-used admin writes when a screen needs them; and the console slice is a Node/vitest contract proof, not a browser test — the Playwright `test:e2e:console` tier is Milestone 23, kept out of root `pnpm test`.
